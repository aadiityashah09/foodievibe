...
// Fetch all desserts
$desserts_result = $conn->query("SELECT * FROM desserts ORDER BY created_at DESC");
?>
...
<head>
    ...
    <title>Desserts - Foodie Vibe</title>
    <style>
...
<body>
    <div class="container">
        <header>
            ...
            <p>Explore sweet dessert recipes</p>
        </header>
        ...
        <?php if ($user_role === 'admin'): ?>
            <a href="admin_desserts.php" class="btn add-recipe-btn">Add New Dessert</a>
        <?php endif; ?>

        <div class="recipe-grid">
            <?php if ($desserts_result && $desserts_result->num_rows > 0): ?>
                <?php while ($dessert = $desserts_result->fetch_assoc()): ?>
                    <div class="recipe-card">
                        <img src="<?= htmlspecialchars($dessert['image']) ?>" alt="<?= htmlspecialchars($dessert['name']) ?>"
                            class="recipe-image">
                        <div class="recipe-info">
                            <h3 class="recipe-title"><?= htmlspecialchars($dessert['name']) ?></h3>
                            <a href="view_dessert.php?id=<?= $dessert['id'] ?>" class="btn">Explore Recipe</a>
                            <?php if ($user_role === 'admin' && $dessert['created_by'] == $user_id): ?>
                                <div class="admin-actions">
                                    <a href="edit_dessert.php?id=<?= $dessert['id'] ?>" class="btn">Edit</a>
                                    <a href="delete_dessert.php?id=<?= $dessert['id'] ?>" class="btn"
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <p>No dessert recipes found. Check back later!</p>
            <?php endif; ?>
        </div>
    </div>
</body>
