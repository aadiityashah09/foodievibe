...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["add_product"])) {
    ...
    if (isset($_FILES['product_image'])) {
        ...
        if (move_uploaded_file($image_tmp, $upload_path)) {
            $sql = "INSERT INTO desserts (name, ingredients, instructions, image) 
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["delete_product"])) {
    $id = $_POST["product_id"];
    $result = $conn->query("SELECT image FROM desserts WHERE id = $id");
    if ($result && $row = $result->fetch_assoc()) {
        if (file_exists($row['image'])) {
            unlink($row['image']);
        }
    }
    $conn->query("DELETE FROM desserts WHERE id = $id");
}

// Fetch all desserts
$result = $conn->query("SELECT * FROM desserts ORDER BY created_at DESC");
?>
...
<head>
    <meta charset="UTF-8">
    <title>Admin Panel - Desserts</title>
    <style>
        body {
...
<body>
    <header>
        <h1>Admin Panel - Desserts</h1>
    </header>

    <section class="admin-section">
        <h2>Add New Dessert</h2>
        <form method="POST" enctype="multipart/form-data" class="admin-form">
            <input type="text" name="product_name" placeholder="Dessert Name" required>
            ...
            <button type="submit" name="add_product">Add Dessert</button>
        </form>
        ...
        <table>
            ...
            <tbody>
                            ...
                            <td>
                                ...
                                <form method="POST" style="display:inline;">
                                    ...
                                    <button type="submit" name="delete_product"
                                        onclick="return confirm('Are you sure you want to delete this dessert?');">
                                        Delete
                                    </button>
                                </form>
                            </td>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="6">No desserts found.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
    ...
    <div id="updateModal" class="modal">
        <h2>Update Dessert</h2>
        <form method="POST" class="admin-form">
            <input type="hidden" name="update_product_id" id="update_product_id">
            <input type="text" name="update_product_name" id="update_product_name" placeholder="Dessert Name" required>
            ...
            <textarea name="update_product_instructions" id="update_product_instructions" placeholder="Instructions"
                required></textarea>
            <button type="submit" name="update_product">Update Dessert</button>
